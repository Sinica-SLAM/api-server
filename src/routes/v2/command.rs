...
pub const SCRIPT_PREFIX: &str = "bash /mnt/md0/nfs_share/PKASR/sinica_asr/api"; // for docker
    ...
    tokio::spawn(async move {
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                set_result_status(&conn, results::Status::Fail, result_model.clone())
                    .await
                    .expect("set result status error");
                set_result_file_path(&conn, e.to_string(), result_model.clone())
                    .await
                    .expect("set result file path error");
            }
        }
